...
def index():
    if current_user.is_authenticated:
        ...
        try:
            votes_a = int(redis.get(option_a) or 0)
            votes_b = int(redis.get(option_b) or 0)
        except RedisError:
            votes_a = '<i>cannot connect to Redis, counter disabled</i>'
            votes_b = '<i>cannot connect to Redis, counter disabled</i>'

        if request.method == 'POST':
            try:
                vote = request.form['vote']
                if vote == 'a':
                    votes_a = redis.incr(option_a)
                else:
                    votes_b = redis.incr(option_b)
            except Exception as ex:
                print(ex)
                votes_a = '<i>An error occured</i>'
                votes_b = '<i>An error occured</i>'
...
def callback():
    ...
    user = User(id_=decoded_state_json['id_'], name=decoded_state_json['name'],
                email=decoded_state_json['email'], profile_pic=decoded_state_json['profile_pic'])
